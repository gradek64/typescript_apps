...
const TitlebarGridList = (props: iProps) => {
   ...
   * on computer memory and on overall perfomance. The solution would be to either limit the amount
   * obsever re-creaction by React.memo() or create just one and keep it refference by React.useRef() => the most optimal
   * solution.
   *
   */
  ...
  }, [updatePage]); //observer is re-created only on updatePate call so some optmization is there
  ...
  useEffect(() => {
    ...
    //in this example we keep creating a new one on updatePage method called so it some optimazation
    if (previousObserver) {
