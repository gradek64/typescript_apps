...
//ts type
const addConst_ts = function (x, y) {
    return x + y;
};
//ts type with es6
const addConst_ts_es6 = (x, y) => {
    return x + y;
};
//ts contextualtype with es6 with only one parameter contextual
const addConst_ts_es6_cont = (x, y) => {
    return x + y;
};
const addConst_ts_es6_interface = (x, y) => {
    return x + y;
};
...
const addNonSpecificReturnConst = (x, y) => x + y;
//ts format always require return type even if doesnt so it takes => void
const addNonSpecificReturnConst_ts = (x, y) => x + y;
//ts format always contextual types are skiped in actual functon and kept in declarion
const addNonSpecificReturnConst_ts_cont = (x, y) => x + y;
const addConst_ts_es6_interface_skipped_return = (x, y) => {
    return x + y;
};
...
//es6 format
const buildNameConst = (firstName, lastName) => {
    if (lastName)
        return firstName + ' ' + lastName;
    else
        return firstName;
...
//typescript format;
const buildName_ts = function name(firstName, lastName) {
    if (lastName)
        return firstName + ' ' + lastName;
    else
        return firstName;
...
const buildName_ts_cont = function name(firstName, lastName) {
    if (lastName)
        return firstName + ' ' + lastName;
    else
        return firstName;
};
const buildName_ts_cont_interface = function name(firstName, lastName) {
    if (lastName)
        return firstName + ' ' + lastName;
    else
        return firstName;
...
//es6 format
const buildNameDefaultConst = (firstName, lastName = 'Smith') => firstName + ' ' + lastName;
//ts format type declaration dont take defaults cause they only define types
const buildNameDefaultConst_ts = (firstName, lastName = 'Smith') => firstName + ' ' + lastName;
//ts contextual typing actual parameters types are skiped since defined in function declaration
const buildNameDefaultConst_ts_cont = (firstName, lastName) => firstName + ' ' + lastName;
const buildName_ts_default_interface = function name(firstName, lastName = 'not known') {
    return firstName + ' ' + lastName;
};
buildName_ts_cont_interface('Greg');
buildName_ts_cont_interface('Greg', 'Gil');
...
const rest_operator_ts = (firstName, ...restofName) => firstName + ' ' + restofName.join(' ');
//ts contextual
const rest_operator_ts_cont = (firstName, ...restofName) => firstName + ' ' + restofName.join(' ');
rest_operator_ts_cont('Greg', 'is', 'cool', 'and', 'even');
const buildName_spread_operator_interface = (firstName, ...otherNames) => {
...
const one_or_other = (value) => value;
one_or_other('Greg');
one_or_other(37);
const buildName_one_or_another_interface = (data) => data;
...
//ts format
const setMeSingnature = function (func) {
    const result = func();
    return result; //allows to return even the return set to void
};
//es6 function singnature
const setMeSingnatureES6 = (func) => {
    const result = func();
    return result; //allows to return even the return set to void
};
